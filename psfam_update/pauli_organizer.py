...
class PauliOrganizer:
    ...
    def __init__(self,m,build=True):
        super().__init__()
        self.m = m
        self.u = False
        self.N = 2**m
    ...
    def initialize_decomposition(self):
        pc = defaultdict(default_dict_int)  # Outer defaultdict creates inner defaultdict(int)
        # for i in range(self.N):
        #     for j in range(self.N):
        #         pc[i][j] = 0  # Inner defaultdict will automatically initialize values to 0
        return pc  # Convert to a regular dictionary if needed
